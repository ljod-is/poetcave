...
class Command(BaseCommand):
    ...
    def remove_duplicates(self):
        ...
        with self.connection.cursor() as cursor:
            ...
            cursor.execute(
                'DELETE FROM `cube_poems_daypoem` WHERE `day` < "2000-01-01"'
            )

            # Bookmark garbage.
            cursor.execute(
                "DELETE FROM `bookmarks` WHERE `poem_id` = 0 OR `user_id` = 0"
            )
            ...
            cursor.execute(
                "DELETE FROM `bookmarks` WHERE `id` IN (%s)"
                % ",".join(str(v) for v in bookmarks_to_delete)
            )
    ...
    def import_users(self):
        existing_user_ids = (
            ",".join([str(i) for i in User.objects.all().values_list("id", flat=True)])
            or "0"
        )
        ...
        with self.connection.cursor() as cursor:
            ...
            for row in dictfetchall(cursor):
                ...
                user.is_superuser = (
                    user.is_staff
                ) = user.is_moderator = user.is_reporter = (row["user"] in superusers)
    ...
    def import_authors(self):
        existing_author_ids = (
            ",".join(
                [str(i) for i in Author.objects.all().values_list("id", flat=True)]
            )
            or "0"
        )
    ...
    def import_poems(self):
        ...
        existing_poem_ids = (
            ",".join([str(i) for i in Poem.objects.all().values_list("id", flat=True)])
            or "0"
        )
        ...
        with self.connection.cursor() as cursor:
            ...
            for row in dictfetchall(cursor):
                ...
                with transaction.atomic():
                    ...
                    # matter, since this script will only be run once and then
                    # never again.
                    Poem.objects.filter(id=poem.id).update(
                        date_updated=awarize(row["last_updated"])
                    )
    ...
    def import_day_poem(self):
        existing_daypoems = "'%s'" % "','".join(
            [
                "%s:%s" % (v["poem_id"], v["day"].strftime("%Y-%m-%d"))
                for v in DayPoem.objects.all().values("poem_id", "day")
            ]
        )
    ...
    def import_bookmarks(self):
        existing_bookmarks = "'%s'" % "','".join(
            [
                "%s:%s" % (b["user_id"], b["poem_id"])
                for b in Bookmark.objects.all().values("poem_id", "user_id")
            ]
        )
        ...
        with self.connection.cursor() as cursor:
            ...
            for row in dictfetchall(cursor):
                ...
                print(
                    "Saving bookmark (user_id %d, poem_id %d)..."
                    % (row["user_id"], row["poem_id"]),
                    end="",
                    flush=True,
                )
                bookmark.save()
    ...
    def import_articles(self):
        ...
        with self.connection.cursor() as cursor:
            existing_article_ids = (
                ",".join(
                    [str(i) for i in Article.objects.all().values_list("id", flat=True)]
                )
                or "0"
            )
            ...
            for row in dictfetchall(cursor):
                ...
                print(
                    'Saving news article "%s"...' % row["fyrirsogn"], end="", flush=True
                )
                article.save()
                print(" done")
    ...
    def configure_private_paths(self):
        ...
        for private_path in paths_to_author_ids:
            ...
            if author.private_path is None:
                print(
                    'Setting private path for author "%s"...' % author,
                    end="",
                    flush=True,
                )
